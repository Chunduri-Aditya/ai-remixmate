...
import { automixPlanner } from "../intelligence/automixPlanner";
...
describe("automixPlanner", () => {
  it("returns all tracks once", () => {
    const plan = automixPlanner([trackA, trackB, trackC]);
    expect(plan.orderedTrackIds.sort()).toEqual(["a", "b", "c"]);
    expect(new Set(plan.orderedTrackIds).size).toBe(3);
  });
});
