...
import { transitionPlanner } from "../intelligence/transitionPlanner";
...
describe("transitionPlanner", () => {
  it("returns valid times and cues", () => {
    const plan = transitionPlanner(trackA, trackB, 4);
    expect(plan.exitTimeSec).toBeGreaterThanOrEqual(0);
    expect(plan.entryTimeSec).toBeGreaterThanOrEqual(0);
    expect(plan.cueSuggestions.length).toBe(2);
  });
});
